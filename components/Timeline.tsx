...
export const Timeline: React.FC<TimelineProps> = ({ currentModel, onModelChange }) => {
  ...
  return (
    <div className="inline-flex items-center bg-gray-800/60 backdrop-blur-sm py-2 px-2 rounded-lg border border-gray-700 max-w-full">
      <div className="flex flex-wrap justify-center items-center gap-x-1 sm:gap-x-2 md:gap-x-3">
        {models.map((model, index) => (
          <React.Fragment key={model.id}>
            <button
              onClick={() => onModelChange(model.id)}
              className={`px-1.5 py-1 text-sm sm:text-base md:text-lg rounded-md transition-all duration-200 focus:outline-none focus:ring-2 focus:ring-indigo-400 flex flex-col items-center ${
